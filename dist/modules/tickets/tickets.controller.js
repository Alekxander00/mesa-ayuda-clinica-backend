...
exports.debugTicket = debugTicket;
...
async function debugTicket(req, res) {
    try {
        const { id } = req.params;
        const user = req.user;
...
async function createTicket(req, res) {
    try {
        ...
        const moduleExists = await prisma_1.prisma.module.findUnique({
        ...
        const ticketTypeExists = await prisma_1.prisma.ticketType.findUnique({
        ...
        const generateUniqueCode = async () => {
            ...
            do {
                ...
                const existingTicket = await prisma_1.prisma.ticket.findUnique({
                    where: { code }
                });
                if (!existingTicket) {
                    return code;
...
async function getTicketMessages(req, res) {
    try {
        const { id } = req.params;
        const messages = await prisma_1.prisma.ticketMessage.findMany({
            where: { ticket_id: id },
            include: {
...
async function addMessageToTicket(req, res) {
    try {
        const { id } = req.params;
